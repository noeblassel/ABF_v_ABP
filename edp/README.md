...
J\cdot n = 0 \text{ on } \partial\Omega .
$$
...
$\rho_\infty \propto \exp\!\big(-\beta(U-\gamma F)\big)$, implying the same
...
We advance by a backward-Euler step in which the biasing nonlinearity is explicit/lagged:
...
K_{\nabla U}=\big[\textstyle\int u\,\nabla U\!\cdot\!\nabla v\big],\qquad
K_{\Delta}=\big[\textstyle\int\nabla u\!\cdot\!\nabla v\big].
$$
...
$A'^{\,n}=(\partial_x U\,\rho^n)^1/\max\!\big((\rho^n)^1,\varepsilon\big)$ ($\varepsilon=10^{-10}$
...
\qquad
B(\rho^n)=\big[\textstyle\int u\,g^n\,\partial_x v\big].
$$
...
$$
b_{\mathrm{ABP}}(\rho^n;\rho^{n+1},v)=\frac{\alpha}{\beta}\int c^n\,\partial_x\!\big(P\rho^{n+1}\big)\,\partial_x v,
\qquad
B(\rho^n)=\frac{\alpha}{\beta}\,S({c^n})P,\quad
S({c^n}):=\big[\textstyle\int c^n\,\partial_x u\,\partial_x v\big].
$$
